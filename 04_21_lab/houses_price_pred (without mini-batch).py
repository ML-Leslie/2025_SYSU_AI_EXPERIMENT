...
# 多层感知机（MLP）类
class MLP:
    def __init__(self, layers:list, activation='relu', learning_rate=0.01, max_iterations=1000):
        """
        初始化多层感知机
    ...
    def run(self, X, Y):
        for i in range(self.max_iterations):
            ...
            # 计算损失
            loss = np.mean((Y_pred.T - Y) ** 2)
            self.losses.append(loss)
            
            if i % 100 == 0:
                print(f"Iteration {i}, Loss: {loss}")
            else:
                print(f"\rIteration {i}, Loss: {loss}", end="")
...
def main():
    ...
    # 创建MLP模型
    mlp = MLP(layers=[X.shape[1], 32, 16, 8, 1], activation='relu', learning_rate=0.025, max_iterations=2000)
