    ...
    for m in range(len(i)):
        for k in range(len(j)):
            ...
            if ifin(myset, new_tuple) == 0:
                print(
                    f"{next_clause_id} R[{myset[i]}{chr(ord('a') + m) if not len(i) == 1 else ''}, {myset[j]}{chr(ord('a') + k) if not len(j) == 1 else ''}]",
                    end="",
                )
                for var, val in var_map.items():
                    print(f"({var} = {val})", end="")
...
def print_proof():
    ...
    sorted_proof_steps = sorted(proof_steps, key=lambda x: myset[x[0]])

    for current, p1, p2, m, k, var_map in sorted_proof_steps:
        print(
        ...
        for i in range(len(clauses)):
            for j in range(len(clauses)):
                if i != j:
                    ...
                    new_bool = unify(tuple_i, tuple_j)
                    ...
                    if () in myset:
                        ...
                        print_proof()
                        return
                    elif new_bool and if_end(myset):
                        print("\n归结完成！\n")
                        print("提取关键路径并重新编号：")
                        print_proof()
                        return
