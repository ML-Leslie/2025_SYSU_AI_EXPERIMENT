...
tuple_3 = ("Green(tony)",)
tuple_4 = ("~Green(john)",)
tuple_5 = ("On(x,yy)","~Green(x)","Green(y)",)
        ...
        for k in range(len(j)):
            ...
            if len(vars_1) == len(vars_2):
                ...
                for var1, var2 in zip(vars_1, vars_2):
                    # 如果两个都是常量(长度>1)且不相等，则标记为跳过
                    if len(var1) > 1 and len(var2) > 1 and var1 != var2:
                        skip_this_pair = True
                        break
                if skip_this_pair:
                    continue
            ...
            if len(vars_1) == len(vars_2):  # 确保变量数量相同
                for var1, var2 in zip(vars_1, vars_2):
                    ...
                    if var1 != var2:
                        if len(var1) == 1 and len(var2) != 1:  # 假设单字符是变量
                            var_map[var1] = var2
                        elif len(var2) == 1 and len(var1) != 1:
                            var_map[var2] = var1
            ...
            #     # 处理变量可能出现在不同位置的情况
            #     new_i = tuple(
            #         item.replace(f"({var})", f"({val})").replace(f",{var})", f",{val})")
            #         for item in new_i
            #     )
            #     new_j = tuple(
            #         item.replace(f"({var})", f"({val})").replace(f",{var})", f",{val})")
            ...
            for var, val in var_map.items():
                ...
                for item in new_i:
                    # 处理所有可能位置
                    new_item = item
                    new_item = new_item.replace(f"({var},", f"({val},")
                    new_item = new_item.replace(f"({var})", f"({val})")
                    new_item = new_item.replace(f",{var},", f",{val},")
                    new_item = new_item.replace(f",{var})", f",{val})")
                    temp_i.append(new_item)
                new_i = tuple(temp_i)
